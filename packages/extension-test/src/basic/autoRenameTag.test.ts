...
>
  ...
  test('bug https://github.com/formulahendry/vscode-auto-rename-tag/issues/542', async () => {
    await createTestFile('script-and-style-bug.html');
    const testCases: TestCase[] = [
      {
        input: `<script|>
</script>`,
  ...
  test('bug https://github.com/formulahendry/vscode-auto-rename-tag/issues/571', async () => {
    await createTestFile('script-bug.html');
    const testCases: TestCase[] = [
      {
        input: `<script |src=""></script>`,
        type: '{backspace}abc',
        expect: `<scriptabcsrc=""></scriptabcsrc=>`
      }
    ];
    await run(testCases, {
      speed: slowSpeed,
